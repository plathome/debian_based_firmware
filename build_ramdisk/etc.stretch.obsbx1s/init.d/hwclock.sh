...
hwclocksh()
{
    [ ! -x /usr/local/sbin/hwclock ] && return 0
	...
	start)
		...
		if /usr/local/sbin/hwclock --rtc=/dev/$HCTOSYS_DEVICE --hctosys $HWCLOCKPARS $BADYEAR; then
	...
	stop|restart|reload|force-reload)
		...
		if /usr/local/sbin/hwclock --rtc=/dev/$HCTOSYS_DEVICE --systohc $HWCLOCKPARS $BADYEAR; then
	...
	show)
	    if [ "$HWCLOCKACCESS" != no ]; then
		/usr/local/sbin/hwclock --rtc=/dev/$HCTOSYS_DEVICE --show $HWCLOCKPARS $BADYEAR
	    fi
	    ;;
	*)
