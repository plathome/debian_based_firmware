...
obsiot_env(){
	...
	fw_setenv miscargs
	fw_setenv efi_file EFI/debian/grubaa64.efi
	...
	fw_setenv efiboot 'echo Booting from GRUB ...; run loadefi; run loadfdt; bootefi ${loadaddr} ${fdt_addr}'
	...
	[ ! `fw_printenv | grep "^bootdev" 2> /dev/null` ] && fw_setenv bootdev rdboot
}

usage(){
	echo "Update uboot environment for OpenBlocks Duo"
	echo
	echo "usage: $(basename $0) [-a]"
	echo
	echo "	-a			OpenBlocks Duo default environment"
