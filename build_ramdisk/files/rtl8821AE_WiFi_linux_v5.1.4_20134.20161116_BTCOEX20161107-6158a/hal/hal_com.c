...
u8  rtw_hal_networktype_to_raid(_adapter *adapter, struct sta_info *psta)
{
	...
	return phydm_rate_id_mapping(&pHalData->odmpriv, psta->wireless_mode, pHalData->rf_type, psta->bw_mode);
#endif
}
...
static u8 rtw_hal_check_pno_enabled(_adapter *adapter)
{
	...
	if (ppwrpriv->wowlan_pno_enable && ppwrpriv->wow_in_resume == _FALSE) {
...
#ifdef CONFIG_PNO_SUPPORT
		if (pwrctl->wow_in_resume == _FALSE &&
...
#ifdef CONFIG_WOWLAN
	if (pwrctl->wowlan_mode == _TRUE &&  pwrctl->wow_in_resume == _FALSE) {
