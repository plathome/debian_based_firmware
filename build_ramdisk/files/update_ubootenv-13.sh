...
obsiot_env(){
	...
	fw_setenv load_kernel 'ext4load mmc 0:8 \${loadaddr} bzImage;ext4load mmc 0:8 0x8000000 ramdisk-wheezy.obsbx1.img.gz'
	...
	fw_setenv bootYocto 'echo "Target:\${target_name}"; run boot_target_cmd_y;'
	fw_setenv boot_target_cmd_y 'run do_flash_os;run do_probe_dfu;run do_compute_target;run mmc-bootargs_y;run load_kernel_y;zboot \${loadaddr}'
	fw_setenv mmc-bootargs_y 'setenv bootargs root=PARTUUID=\${uuid_rootfs} \${bootargs_console} \${bootargs_debug} systemd.unit=\${bootargs_target}.target hardware_id=\${hardware_id} g_multi.iSerialNumber=\${serial#}'
	fw_setenv load_kernel_y 'fatload mmc 0:7 \${loadaddr} vmlinuz'
